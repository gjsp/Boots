...
Partial Class ucLFLCompare
        ...
        Try
            ...
            Else
                ...
                If ds.Tables(clsBts.reportPart.Total.ToString).Rows(0)("store_name").ToString.Contains("<br/>") Then
                    brTopic = "<br/>"
                End If
                lblTopicTable.Text = String.Format(clsHtml.htmlLFLCompareTopic, ReportType + " LFL Compare Report", brTopic)
    ...
    Protected Sub dlItem_ItemDataBound(sender As Object, e As System.Web.UI.WebControls.DataListItemEventArgs) Handles dlItem.ItemDataBound
        ...
        sbHeadItem.Append(e.Item.DataItem("topicyear").ToString) '0
        sbHeadItem.Append(l + e.Item.DataItem("store_name").ToString)
        ...
        If e.Item.DataItem("store_name").ToString.Contains("Total") Then
            tblClass = True
        End If
        Dim htmlHead As String = String.Format(clsHtml.htmlLFLCompareItem(clsHtml.tablePart.head, tblClass), sbHeadItem.ToString.Split(l))
        ...
        For i As Integer = 0 To drv.Row.ItemArray.Count - 1
            ...
            End If
        Next
        Dim htmlBody As String = String.Format(clsHtml.htmlLFLCompareItem(clsHtml.tablePart.body), sbBodyItem.ToString.Split(l))
        ...
        Dim htmlFoot As String = clsHtml.htmlLFLCompareItem(clsHtml.tablePart.foot)
    ...
    Protected Sub dlTotal_ItemDataBound(sender As Object, e As System.Web.UI.WebControls.DataListItemEventArgs) Handles dlTotal.ItemDataBound
        ...
        If e.Item.DataItem("store_name") = clsBts.reportPart.Total.ToString + "Yoy" Or e.Item.DataItem("store_name").ToString = "" Then Exit Sub 'datatable มีrow ที่เป็น yoy ไม่ต้องโชว์
        ...
        sbHeadItem.Append(strNone) '0
        sbHeadItem.Append(l + "% Growth") '1
        sbHeadItem.Append(l + strNone) '2
        sbHeadItem.Append(l + strNone) '3
        sbHeadItem.Append(l + strNone) '4
        sbHeadItem.Append(l + strNone) '5
        'sbHeadItem.Append(l + e.Item.DataItem("yoy_growth").ToString) '6
        'sbHeadItem.Append(l + e.Item.DataItem("lfl_growth").ToString) '7
        Dim htmlHead As String = String.Format(clsHtml.htmlGrowthLFLCompare(clsHtml.tablePart.head), sbHeadItem.ToString.Split(l))
        ...
        For i As Integer = 0 To drv.Row.ItemArray.Count - 1
            If drv.DataView.Table.Columns(i).ColumnName.Contains("Sum") Then
                If Not (drv.DataView.Table.Columns(i).ColumnName = "Sumtotalarea" Or drv.DataView.Table.Columns(i).ColumnName = "Sumsalearea") Then
                    ...
                    sbBodyItem.Append(l + ClsManage.convert2PercenLFLGrowth(valSumItem.ToString) + _
                                      l + "" + _
                                      l + "")
                End If
            End If
        Next
        Dim htmlBody As String = String.Format(clsHtml.htmlGrowthLFLCompare(clsHtml.tablePart.body), sbBodyItem.ToString.Split(l))

        'Foot
        'Dim sbFootItem As New StringBuilder
        'sbFootItem.Append(e.Item.DataItem("yoy_loss_growth").ToString) '0
        'sbFootItem.Append(l + e.Item.DataItem("lfl_loss_growth").ToString) '1
        'Dim htmlFoot As String = String.Format(clsHtml.htmlGrowthLFLCompare(clsHtml.tablePart.foot), sbFootItem.ToString.Split(l))
        Dim htmlFoot As String = clsHtml.htmlGrowthLFLCompare(clsHtml.tablePart.foot)
